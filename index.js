...
(() => {
    ...
    if (roulette === null)
        throw new Error("No canvas with name id 'roulette' is found.");
    roulette.width = WIDTH;
    roulette.height = HEIGHT;
    const ctx = roulette.getContext("2d");
    if (ctx === null)
        throw new Error("2D contex is not supported.");
    console.log(ctx);
    const textInput = document.getElementById('textInput');
    if (textInput === null)
        throw new Error("No text area with name id 'textInput' is found.");
    const drawButton = document.getElementById('drawButton');
    if (drawButton === null)
        throw new Error("No button with name id 'drawButton' is found.");
    ...
    const getSliceValueAtAngle = (targetAngle) => {
        const totalLines = lines.length;
        const anglePerSlice = (2 * Math.PI) / totalLines;
